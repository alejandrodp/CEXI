...
bitset<32> MemoryManager::allocate(int size) {
    ...
    for(;currentPage != nullptr; currentPage = currentPage->next){
        ...
        if(currentPage->used + size <= PAGESIZE) {  //Check if there is enough space in any of the existing pages
            spaceFound = true;
    ...
    if(!spaceFound){ //Create new page if needed
        ...
        currentPage->next->start = (currentPage->start + PAGESIZE); //The page should start where the last one ends
...
MemoryManager * MemoryManager::getInstance() {
    if(instance == nullptr){
        instance = new MemoryManager(0x3c);
    }
    return instance;
}
...
void MemoryManager::loadPage(int page) {
    ...
    DiskAccess::writeToDisk(leastUsed->page, leastUsed->start, PAGESIZE); //Write the content of the page  to take down into the disk
    ...
    if(toLoad->used > 0) { //Check for something to load
        ...
        toLoad->start = memcpy(leastUsed->start, dataToLoad, PAGESIZE); //Update the first address of the page that is being loaded
        ...
        memset(toLoad->start, 0, PAGESIZE); //Set all bits in the position to 0, as if called with calloc
    }
